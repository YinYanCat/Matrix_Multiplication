...
Matrix<double> genDiagonal(std::size_t n) {
...
Matrix<double> genUpperT(std::size_t n) {
    Matrix<double> result(n,n);
    std::random_device rd;
    std::mt19937 gen(rd());
    std::uniform_real_distribution<double> dist(0.0, 1.0);
...
Matrix<double> genComplete(std::size_t n) {
    Matrix<double> result(n,n);
    std::random_device rd;
    std::mt19937 gen(rd());
    std::uniform_real_distribution<double> dist(0.0, 1.0);
...
void genDiagonalM(std::size_t n, int quantity) {
